...
def norm_code(code):
    ...
    read as float64, so 1234 arrives as "1234.0" and never matches the cache.
    The orchestrator and the scraper MUST normalise identically or every
    lookup misses and IR Distance comes out empty.
    """
...
class RBSScraper:
    ...
    def _fetch_single_route(self, source, destination):
        ...
        if cached and cached['status'] == 'SUCCESS':
            ...
            self._mark(source, destination, "CACHE")
    ...
    def get_routes_batch(self, od_pairs, max_workers=MAX_WORKERS):
        ...
        unique = list({(norm_code(s), norm_code(d)) for s, d in od_pairs})
        results = {}
